...
function validate() {
    ...
    if (month.value.length == 0) monthInvalid.innerText = "Can't be blank";
    if (year.value.length == 0) yearInvalid.innerText = "Can't be blank";
}
...
month.addEventListener("keyup", (event)=>{
    if (month.value.length >= 1) monthInvalid.innerText = "";
    if (month.value.length == 2) month.nextElementSibling.focus();
});
...
year.addEventListener("keyup", (event)=>{
    if (year.value.length >= 1) yearInvalid.innerText = "";
});
